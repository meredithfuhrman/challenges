...
#Joining the hashes to make this simpler
twitter = twitter_data[0]
(1..(twitter_data.length-1)).each {|i| twitter.merge!(twitter_data[i])}

#Each username followed by its description. If there is no description (i.e., nil then print out "NA" instead):
puts
puts "Description:"
twitter.each do |key, value|
...
puts
puts "Total Number of Followers:"
twitter.each {|key, value| puts "#{key}: #{value["number of followers"]}"}

#Each username and the length of their latest tweet:
puts
puts "Length of Latest Tweet:"
twitter.each {|key, value| puts "#{key}: #{value["latest tweet"].length}"}


#Each username followed by the total character count used in that user's last twenty tweets
puts
puts "Character Count of Last 20 Tweets:"
twitter.each do |key, value|
  sum = value["last twenty tweets"].map {|element| element.length}.reduce(:+)
  puts "#{key}: #{sum}"
end
...
puts
puts "User with Most Followers:"
most = twitter.max_by {|key, value| value["number of followers"]}
puts most[0]

#Which user has the most friends?
puts
puts "User with Most Friends:"
most = twitter.max_by {|key, value| value["number of friends"]}
puts most[0]

#Which user has the greatest number of tweets?
puts
puts "User with Greatest Number of Tweets:"
most = twitter.max_by {|key, value| value["number of tweets"]}
puts most[0]

#Which users have a description listed in twitter_data?
puts
puts "Users with Description Listed:"
twitter.each {|key, value| puts "#{key}" unless value["description"] == nil}


#Which users have a location listed in twitter_data?
puts
puts "Users with Location Listed:"
twitter.each {|key, value| puts "#{key}" unless value["location"] == nil}
